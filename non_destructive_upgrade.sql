...
-- Bookings RLS hardening for guest ownership + route-safe guest updates.
DO $$
BEGIN
  ALTER TABLE public.bookings ENABLE ROW LEVEL SECURITY;

  DROP POLICY IF EXISTS "Active staff view all bookings" ON public.bookings;
  DROP POLICY IF EXISTS "Guests view own bookings" ON public.bookings;
  DROP POLICY IF EXISTS "Active staff can insert bookings" ON public.bookings;
  DROP POLICY IF EXISTS "Active staff can update bookings" ON public.bookings;
  ...
  CREATE POLICY "Active staff can insert bookings" ON public.bookings FOR INSERT
  WITH CHECK (
    is_user_active(auth.uid())
  ...
  CREATE POLICY "Active staff can update bookings" ON public.bookings FOR UPDATE
  USING (
    is_user_active(auth.uid())
    AND user_has_role(auth.uid(), 'receptionist')
  )
...
-- Rooms: manager/owner can INSERT (add room) for Room Management screen
DO $$
BEGIN
  DROP POLICY IF EXISTS "Staff management can insert rooms" ON public.rooms;
EXCEPTION WHEN undefined_object THEN
  NULL;
END $$;
CREATE POLICY "Staff management can insert rooms" ON public.rooms
FOR INSERT
WITH CHECK (
  is_user_active(auth.uid())
  AND (
...
-- Room types: manager/owner can UPDATE (e.g. price) for Room Management screen
DO $$
BEGIN
  DROP POLICY IF EXISTS "Staff management can update room types" ON public.room_types;
EXCEPTION WHEN undefined_object THEN
  NULL;
END $$;
CREATE POLICY "Staff management can update room types" ON public.room_types
FOR UPDATE
USING (
  is_user_active(auth.uid())
  AND (
...
-- Menu items: manager/owner can INSERT/UPDATE/DELETE (kitchen product catalog)
DO $$
BEGIN
...
EXCEPTION WHEN undefined_object THEN
  NULL;
END $$;
CREATE POLICY "Management can manage menu items" ON public.menu_items
FOR ALL
USING (
  is_user_active(auth.uid())
  AND (
...
ALTER TABLE public.mini_mart_items ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT now();
...
-- Staff activities table for operational audit trail (Room Management, Inventory, Kitchen, etc.)
...
CREATE INDEX IF NOT EXISTS idx_staff_activities_staff_profile_id ON public.staff_activities(staff_profile_id);
CREATE INDEX IF NOT EXISTS idx_staff_activities_created_at ON public.staff_activities(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_staff_activities_department ON public.staff_activities(department);
ALTER TABLE public.staff_activities ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Staff insert own activity" ON public.staff_activities;
CREATE POLICY "Staff insert own activity" ON public.staff_activities
FOR INSERT
WITH CHECK (auth.uid() = staff_profile_id);

DROP POLICY IF EXISTS "Manager owner read staff activities" ON public.staff_activities;
CREATE POLICY "Manager owner read staff activities" ON public.staff_activities
FOR SELECT
USING (
  EXISTS (
    SELECT 1 FROM public.profiles p
...
DROP POLICY IF EXISTS "Staff read own activities" ON public.staff_activities;
CREATE POLICY "Staff read own activities" ON public.staff_activities
FOR SELECT
USING (auth.uid() = staff_profile_id);
